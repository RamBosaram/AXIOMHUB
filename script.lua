...
do
    ...
    if not Executor.hui and get_hidden_gui then
...
function SilentAim:fastPredict(target, offset)
    ...
    return hrp.Position
        + (vel * Vector3.new(0, 0.5, 0)) * (offset / 15)
        + moveDir * offset
end
...
function SilentAim:findGunRemote(gun)
    ...
    if knifeLocal then
        ...
        if createBeam then
            local rf = createBeam:FindFirstChild("RemoteFunction")
...
function SilentAim:shoot()
    ...
    if kind == "function" then
        success, err = pcall(function()
            return remote:InvokeServer(1, aim, "AH2")
        end)
    else
        success, err = pcall(function()
            remote:FireServer(1, aim, "AH2")
        end)
    end
...
local function namecallHandler(self, ...)
    ...
    if State.SilentAim.Enabled and (method == "FireServer" or method == "InvokeServer") then
        local target = getMurderer() or getFallbackTarget()
        if target then
            local aim = SilentAim:computeAimPoint(target)
            if aim then
                local args = {...}
...
if hookmetamethod then
    ...
    oldIndex = hookmetamethod(game, "__index", function(self, key)
        if State.SilentAim.Enabled and self == Mouse then
            if key == "Hit" then
                local m = getMurderer() or getFallbackTarget()
                if m then
